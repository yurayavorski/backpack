...
const adjustTypographyAndroidRn = prop => {
  let adjustedValue = prop.value;
  if (prop.category === 'line-heights') {
    ...
    // We similarly wish to increase the line spacing by a factor of 0.13.
    // However, RN has only the concept of line-height,
    // so we need increase only the spacing portion of the line-height
    const lineSpacing = parseFloat(prop.value) - correspondingFontSize;
    const adjustment = lineSpacing * 1.13;
    adjustedValue = correspondingFontSize + adjustment;
  }
...
const adjustTypographyAndroid = prop => {
  let adjustedValue = prop.value;
  if (prop.category === 'line-heights') {
    adjustedValue = parseFloat(prop.value) * 1.13;
  }
